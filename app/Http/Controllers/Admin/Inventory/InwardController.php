...
class InwardController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            $query = Inward::with(['inwardDetails.packingListDetail.product', 'inwardDetails.room', 'inwardDetails.rack', 'inwardDetails.slot', 'inwardDetails.pallet', 'client'])
                            ->where(function ($query) {
                                $query->whereHas('inwardDetails', function ($q) {
                                    $q->groupBy('status'); 
                                });
                            })
            ...
            return DataTables::eloquent($query)
                ->filter(function ($query) use ($request) {
                    ...
                    if ($search != '') {
                        $query->where(function ($q) use ($search) {
                            $q->where('doc_no', 'like', "%{$search}%")
                                ...
                                ->orWhereHas('inwardDetails.room', function ($q2) use ($search) {
                                    $q2->where('name', 'like', "%{$search}%");
                                })
                                ->orWhereHas('inwardDetails.rack', function ($q2) use ($search) {
                                    $q2->where('name', 'like', "%{$search}%");
                                })
                                ->orWhereHas('inwardDetails.slot', function ($q2) use ($search) {
                                    $q2->where('name', 'like', "%{$search}%");
                                })
                                ->orWhereHas('inwardDetails.pallet', function ($q2) use ($search) {
                                    $q2->where('name', 'like', "%{$search}%");
                                })
                                ->orWhereHas('inwardDetails.packingListDetail.product', function ($q2) use ($search) {
                                    $q2->where('product_description', 'like', "%{$search}%");
                                })
                                ->orWhereHas('inwardDetails.packingListDetail', function ($q2) use ($search) {
                    ...
                    if ($request->filled('status')) {
                        $query->whereHas('inwardDetails', function ($q) use ($request) {
                                $q->where('status', $request->status);
                            });
                    }
                })
                // ->addColumn('inward_items', function ($inward) {
                //     if ($inward->inwardDetails->isEmpty()) {
                //         return '<tr><td colspan="7" class="text-center">No Items</td></tr>';
                //     }

                //     $rows = '';
                //     foreach ($inward->inwardDetails as $item) {
                //         $rows .= '<tr>';
                ...
                // })
                ->editColumn('doc_date', function ($inward) {
                    return $inward->formatDate('doc_date');
                })
                ->addColumn('no_of_items', function ($inward) {
                    return $inward->inwardDetails->count();
                })
                ->addColumn('status', function ($inward) {
                    return ucfirst($inward->status);
                })
                ->addColumn('action', function ($inward) {
                    $act = '';
                    if($inward->status_list->contains('Created')) {
                        $act .= '<a href="'.route('admin.inventory.inward.edit', $inward->inward_id).'" class="btn btn-warning btn-sm" ><i class="fas fa-edit" ></i></a>';
                    }
                    $act .= '<a href="' . route('admin.inventory.inward.print', $inward->inward_id) . '" target="_blank" class="btn btn-sm btn-print"><i class="fas fa-print"></i></a>';
                    $act .= '&nbsp;<a href="' . route('admin.inventory.inward.view', $inward->inward_id) . '" class="btn btn-sm btn-view"><i class="fas fa-eye"></i></a>';
                    return $act;
                })
                ->rawColumns(['doc_no', 'inward_items', 'action'])
                ->addIndexColumn()
                ->make(true);
        }

        return view('admin.inventory.inward.index', compact('clients', 'statuses')); 
    }
