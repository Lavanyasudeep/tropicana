...
<div class="page-sub-header">
    ...
    <div class="action-btns">
        ...
        <a href="{{ route('admin.master.sales.customer.create') }}" class="btn btn-primary btn-sm" title="create">
...
<!-- Advance Filter -->
<div class="page-advance-filter" id="customerAdvFilterPanel">
    <form id="customerAdvFilterForm">
        <div class="row">
            <div class="col-md-3">
...
<div class="card page-list-panel">
    <div class="card-body">
        <table id="customerListTable" class="page-list-table">
            <thead>
                <tr>
                    <th>#</th>
                    <th>Photo</th>
...
$(document).ready(function() {
    ...
    $('#hdrAdvFilterBtn').on("click", function(e) {
        e.preventDefault();
        $('#customerAdvFilterPanel').toggle();
    });

    $('#cancelFltrBtn').on("click", function () {
        const form = $('#customerAdvFilterForm');
    ...
    $('#closeFltrBtn').on("click", function () {
        const form = $('#customerAdvFilterForm');
        form.find('input, select').val('');
        $('#customerAdvFilterPanel').hide();
    });

    $('#customerAdvFilterForm').on("submit", function(e) {
        e.preventDefault();
        table.ajax.reload();
    });
    ...
    let table = $('#customerListTable').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: '{{ route("admin.master.sales.customer.index") }}',
            data: function (d) {
                d.quick_search = $('#fltrSearchBox').val();
            }
        },
        columns: [
            { data: 'customer_id', name: 'customer_id', width: '8%' },
            { data: 'photo', name: 'photo', width: '8%' },
            { data: 'customer_name', name: 'customer_name', width: '10%' },
            { data: 'state.state_name', name: 'state.state_name', width: '15%', defaultContent: '' },
            { data: 'district_name', name: 'district_name', width: '15%' },
    ...
    $('#customerListTable').on("submit", function(e) {
        e.preventDefault();
        table.ajax.reload();
    });
    ...
    $('#customerListTable').on('click', '.toggle-status', function () {
        const button = $(this);
        const id = button.data('id');

        $.post('{{ route('admin.master.sales.customer.toggle-status') }}', {
            _token: '{{ csrf_token() }}',
            id: id
        }, function (res) {
            if (res.success) {
