...
<div class="page-sub-header">
    ...
    <div class="action-btns">
        ...
        <a href="{{ route('admin.master.hr.user.create') }}" class="btn btn-primary btn-sm" title="create">
...
<!-- Advance Filter -->
<div class="page-advance-filter" id="userAdvFilterPanel">
    <form id="userAdvFilterForm">
        <div class="row">
            <div class="col-md-3">
...
<div class="card page-list-panel">
    <div class="card-body">
        <table id="userListTable" class="page-list-table">
            <thead>
                <tr>
                    <th>#</th>
                    <th>Employee Name</th>
...
$(document).ready(function() {
    ...
    $('#hdrAdvFilterBtn').on("click", function(e) {
        e.preventDefault();
        $('#userAdvFilterPanel').toggle();
    });

    $('#cancelFltrBtn').on("click", function () {
        const form = $('#userAdvFilterForm');
    ...
    $('#closeFltrBtn').on("click", function () {
        const form = $('#userAdvFilterForm');
        form.find('input, select').val('');
        $('#userAdvFilterPanel').hide();
    });

    $('#userAdvFilterForm').on("submit", function(e) {
        e.preventDefault();
        table.ajax.reload();
    });
    ...
    let table = $('#userListTable').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: '{{ route("admin.master.hr.user.index") }}',
            data: function (d) {
        ...
        columns: [
            ...
            { data: 'employee.designation.designation_name', name: 'employee.designation.designation_name', width: '10%', defaultContent: '' },
            { data: 'branch.branch_name', name: 'branch.branch_name', width: '10%' },
            { data: 'role.role_name', name: 'role.role_name', width: '8%' },
    ...
    $('#userListTable').on("submit", function(e) {
        e.preventDefault();
        table.ajax.reload();
    });
    ...
    $('#userListTable').on('click', '.toggle-status', function () {
        const button = $(this);
        const id = button.data('id');

        $.post('{{ route('admin.master.hr.user.toggle-status') }}', {
            _token: '{{ csrf_token() }}',
            id: id
        }, function (res) {
            if (res.success) {
