...
<div class="page-sub-header">
    ...
    <div class="action-btns">
        ...
        <a href="{{ route('admin.master.general.role.create') }}" class="btn btn-primary btn-sm" title="create">
...
<!-- Advance Filter -->
<div class="page-advance-filter" id="roleAdvFilterPanel">
    <form id="roleAdvFilterForm">
        <div class="row">
            <div class="col-md-3">
...
<div class="card page-list-panel">
    <div class="card-body">
        <table id="roleListTable" class="page-list-table">
            <thead>
                <tr>
                    <th>#</th>
                    <th>Role Name</th>
...
$(document).ready(function() {
    ...
    $('#hdrAdvFilterBtn').on("click", function(e) {
        e.preventDefault();
        $('#roleAdvFilterPanel').toggle();
    });

    $('#cancelFltrBtn').on("click", function () {
        const form = $('#roleAdvFilterForm');
    ...
    $('#closeFltrBtn').on("click", function () {
        const form = $('#roleAdvFilterForm');
        form.find('input, select').val('');
        $('#roleAdvFilterPanel').hide();
    });

    $('#roleAdvFilterForm').on("submit", function(e) {
        e.preventDefault();
        table.ajax.reload();
    });
    ...
    let table = $('#roleListTable').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: '{{ route("admin.master.general.role.index") }}',
            data: function (d) {
                d.quick_search = $('#fltrSearchBox').val();
            }
        },
        columns: [
            { data: 'role_id', name: 'role_id', width: '8%' },
            { data: 'role_name', name: 'role_name', width: '8%' },
            { data: 'short_name', name: 'short_name', width: '15%' },
            { data: 'Status', name: 'Status', width: '8%' },
            { data: 'actions', name: 'actions', orderable: false, searchable: false, width: '13%' },
        ]
    });

    $('#roleListTable').on("submit", function(e) {
        e.preventDefault();
        table.ajax.reload();
    });
    ...
    $('#roleListTable').on('click', '.toggle-status', function () {
        const button = $(this);
        const id = button.data('id');

        $.post('{{ route('admin.master.general.role.toggle-status') }}', {
            _token: '{{ csrf_token() }}',
            id: id
        }, function (res) {
            if (res.success) {
