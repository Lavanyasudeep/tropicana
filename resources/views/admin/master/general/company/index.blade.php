...
<div class="page-sub-header">
    ...
    <div class="action-btns">
        ...
        <a href="{{ route('admin.master.general.company.create') }}" class="btn btn-primary btn-sm" title="create">
...
<!-- Advance Filter -->
<div class="page-advance-filter" id="companyAdvFilterPanel">
    <form id="companyAdvFilterForm">
        <div class="row">
            <div class="col-md-3">
...
<div class="card page-list-panel">
    <div class="card-body">
        <table id="companyListTable" class="page-list-table">
            <thead>
                <tr>
                    <th>#</th>
                    <th>logo</th>
...
$(document).ready(function() {
    ...
    $('#hdrAdvFilterBtn').on("click", function(e) {
        e.preventDefault();
        $('#companyAdvFilterPanel').toggle();
    });

    $('#cancelFltrBtn').on("click", function () {
        const form = $('#companyAdvFilterForm');
    ...
    $('#closeFltrBtn').on("click", function () {
        const form = $('#companyAdvFilterForm');
        form.find('input, select').val('');
        $('#companyAdvFilterPanel').hide();
    });

    $('#companyAdvFilterForm').on("submit", function(e) {
        e.preventDefault();
        table.ajax.reload();
    });

    let table = $('#companyListTable').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: '{{ route("admin.master.general.company.index") }}',
            data: function (d) {
                d.quick_search = $('#fltrSearchBox').val();
            }
        },
        columns: [
            { data: 'company_id', name: 'company_id', width: '8%' },
            { data: 'logo', name: 'logo', width: '8%' },
            { data: 'company_name', name: 'company_name', width: '15%' },
    ...
    $('#companyListTable').on("submit", function(e) {
        e.preventDefault();
        table.ajax.reload();
    });
    ...
    $('#companyListTable').on('click', '.toggle-status', function () {
        const button = $(this);
        const id = button.data('id');

        $.post('{{ route('admin.master.general.company.toggle-status') }}', {
            _token: '{{ csrf_token() }}',
            id: id
        }, function (res) {
            if (res.success) {
