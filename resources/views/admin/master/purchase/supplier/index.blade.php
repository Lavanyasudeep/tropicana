...
<div class="page-sub-header">
    ...
    <div class="action-btns">
        ...
        <a href="{{ route('admin.master.purchase.supplier.create') }}" class="btn btn-primary btn-sm" title="create">
...
<!-- Advance Filter -->
<div class="page-advance-filter" id="supplierAdvFilterPanel">
    <form id="supplierAdvFilterForm">
        <div class="row">
            <div class="col-md-3">
...
<div class="card page-list-panel">
    <div class="card-body">
        <table id="supplierListTable" class="page-list-table">
            <thead>
                <tr>
                    <th>#</th>
                    <th>Name</th>
...
$(document).ready(function() {
    ...
    $('#hdrAdvFilterBtn').on("click", function(e) {
        e.preventDefault();
        $('#supplierAdvFilterPanel').toggle();
    });

    $('#cancelFltrBtn').on("click", function () {
        const form = $('#supplierAdvFilterForm');
    ...
    $('#closeFltrBtn').on("click", function () {
        const form = $('#supplierAdvFilterForm');
        form.find('input, select').val('');
        $('#supplierAdvFilterPanel').hide();
    });

    $('#supplierAdvFilterForm').on("submit", function(e) {
        e.preventDefault();
        table.ajax.reload();
    });

    let table = $('#supplierListTable').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: '{{ route("admin.master.purchase.supplier.index") }}',
            data: function (d) {
                d.quick_search = $('#fltrSearchBox').val();
            }
        },
        columns: [
            { data: 'supplier_id', name: 'supplier_id', width: '8%' },
            { data: 'supplier_name', name: 'supplier_name', width: '15%' },
            { data: 'state.state_name', name: 'state.state_name', width: '10%', defaultContent: '' },
            { data: 'district_name', name: 'district_name', width: '10%' },
            { data: 'contact_person', name: 'contact_person', width: '15%' },
    ...
    $('#supplierListTable').on("submit", function(e) {
        e.preventDefault();
        table.ajax.reload();
    });
    ...
    $('#supplierListTable').on('click', '.toggle-status', function () {
        const button = $(this);
        const id = button.data('id');

        $.post('{{ route('admin.master.purchase.supplier.toggle-status') }}', {
            _token: '{{ csrf_token() }}',
            id: id
        }, function (res) {
            if (res.success) {
