...
<div class="page-sub-header">
    ...
    <div class="action-btns" >
        ...
        <a href="{{ route('admin.accounting.payment.create') }}" class="btn btn-primary btn-sm" title="create"><i class="fas fa-plus" ></i> Create</a>
    </div>
</div>

<!-- Advance Filter -->
<div class="page-advance-filter" id="paymentAdvFilterPanel" >
    <form id="paymentAdvFilterForm" >
        <div class="row">
            <div class="col-md-3" >
...
<div class="card page-list-panel" >
    <div class="card-body">
        <table id="paymentListTable" class="page-list-table">
            <thead>
                <tr>
                    <th>Doc. No.</th>
                    <th>Doc. Date</th>
...
$(document).ready(function() {
    ...
    $(document).on("click", '#hdrAdvFilterBtn', function(e) {
        e.preventDefault();
        if($('#paymentAdvFilterPanel').is(':visible'))
            $('#paymentAdvFilterPanel').hide();
        else
            $('#paymentAdvFilterPanel').show();
    });

    $(document).on("click", "#cancelFltrBtn", function () {
        const form = $('#paymentAdvFilterForm');
    ...
    $(document).on("click", "#closeFltrBtn", function () {
        ...
        document.getElementById('paymentAdvFilterPanel').style.display = 'none';
    });

    $(document).on("submit", '#paymentAdvFilterForm', function(e) {
        e.preventDefault();
        table.ajax.reload();
    });
    /***** End : Advance Filter *****/

    let table =  $('#paymentListTable').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: '{{ route("admin.accounting.payment.index") }}',
            data: function (d) {
    ...
    $(document).on("submit", '#paymentListTable', function(e) {
        e.preventDefault();
        table.ajax.reload();
    });
