<div class="modal fade" id="settlementModal" tabindex="-1" aria-labelledby="settlementModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg" style="width:80%; max-width:80%;" >
        <div class="modal-content">
            ...
            <form method="POST" action="{{ route('admin.accounting.payment.settle', $payment->payment_voucher_id) }}" id="settlementForm">
                @csrf
                    ...
                    <table class="page-list-table" id="settlementTable">
                      ...
                      <tbody>
                        <tr class="advance_account">
                            ...
                            <td><input type="hidden" name="settlement[ledger_account][]" value="{{ $payment->to_account_code }}" >{{ $payment->toAccount->account_name }}</td>
                            <td><input type="hidden" name="settlement[analytical_code][]" value="{{ $payment->to_analytical_code }}" >{{ $payment->analytical_code }}</td>
                            <td><input type="hidden" name="settlement[narration][]" value="" >{{ $payment->narration }}</td>
                            <td><input type="hidden" name="settlement[debit][]" value="0" class="text-right" readonly></td>
                            <td><input type="text" name="settlement[credit][]" value="{{ $payment->requested_amount }}" class="text-right" readonly></td>
                            <td></td>
                        </tr>
                        <tr class="dr_account">
                            <td><b><span class="cr_account_label" >Cash/Bank</span> A/c</b></td>
                            <td><input type="hidden" name="settlement[ledger_account][]" value="{{ $payment->from_account_code }}" >{{ $payment->fromAccount->account_name }}</td>
                            <td><input type="hidden" name="settlement[analytical_code][]" value="{{ $payment->from_analytical_code }}" >{{ $payment->analytical_code }}</td>
                            <td><input type="hidden" name="settlement[narration][]" value="" >{{ $payment->narration }}</td>
                            <td><input type="text" name="settlement[debit][]" value="{{ $payment->requested_amount }}" class="text-right" readonly></td>
...
<script>
const advanceAmount = {{ $payment->requested_amount ?? 0 }};
...
document.getElementById('addSettlementRow').addEventListener('click', function() {
  ...
  const requestedAmount = {{ $payment->requested_amount ?? 0 }};
