...
<div class="page-sub-header">
    ...
    <div class="action-btns" >
        <a href="{{ route('admin.sales.sales-invoice.edit', $invoice->sales_invoice_id) }}" class="btn btn-edit"><i class="fas fa-edit"></i> Edit</a>
        <a href="{{ route('admin.sales.sales-invoice.print', $invoice->sales_invoice_id) }}" target="_blank" class="btn btn-print" ><i class="fas fa-print"></i> Print</a>
    ...
    <div class="action-status" >
        ...
        <select id="change_status_select" >
            ...
            @if(!in_array('approved', $invoice->statusUpdates->pluck('column_value')->toArray()))
                <option value="approved">Approved</option>
            @endif

            @if(!in_array('rejected', $invoice->statusUpdates->pluck('column_value')->toArray()))
                <option value="rejected">Rejected</option>
            @endif

            @if(!in_array('cancelled', $invoice->statusUpdates->pluck('column_value')->toArray()))
    ...
    <div class="tab-content">
                        ...
                        <div class="col-md-4">
                            <div class="pform-panel" style="min-height:120px;">
                                <div class="pform-row">
                                    ...
                                    <div class="pform-value">{{ $invoice->doc_no }}</div>
                                </div>
                                <div class="pform-row">
                                    <div class="pform-label">Doc. Date</div>
                                    <div class="pform-value">{{ $invoice->doc_date }}</div>
                                </div>
                                <div class="pform-row">
                                    <div class="pform-label">Status</div>
                                    <div class="pform-value">{{ $invoice->status ?? '' }}</div>
                                </div>
                        ...
                        <div class="col-md-4">
                            <div class="pform-panel" style="min-height:120px;">
                                <div class="pform-row">
                                    ...
                                    <div class="pform-value">{{ optional($invoice)->customer?->customer_name ?? '' }}</div>
                                </div>
                                <div class="pform-row">
                                    <div class="pform-label">Contact No.</div>
                                    <div class="pform-value">{{ optional($invoice)->customer?->phone_number ?? '' }}</div>
                                </div>
                                <div class="pform-row">
                                    <div class="pform-label">Address</div>
                                    <div class="pform-value">{{ $invoice->customer?->main_address ?? '' }}</div>
                                </div>
                                <div class="pform-clear" ></div>
                            </div>
                        </div>
                        ...
                        <div class="col-md-4">
                            <div class="pform-panel" style="min-height:120px;">
                                <div class="pform-row" >
                                    ...
                                    <div class="pform-value w100p" >{{ $invoice->remarks }}</div>
                                </div>
                                <div class="pform-clear" ></div>
                            </div>
                        </div>
                        ...
                        <div class="col-md-5 offset-md-7">
                            <table class="table table-striped page-list-table">
                                <tbody>
                                    ...
                                    <tr>
                                        <td>IGST</td>
                                        <td class="text-right">{{ $invoice->igst_amount }}</td>
                                    </tr>
                                    <tr>
                                        <td><b>Grand Total</b></td>
                                        <td class="text-right">{{ $invoice->grand_amount }}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
        ...
        <div class="tab-pane fade" id="invoiceStatus" role="tabpanel">
            <div class="card">
                ...
                <div class="card-body">
                    @forelse($invoice->statusUpdates as $log)
...
<script>
    $(document).ready(function() {
        ...
        $('#change_status_select').val('{{$invoice->status}}');
        ...
        $('#change_status_select').on('change',function(){
            ...
            showConfirmationModal(
                ...
                function () { // Yes
                    ...
                    $.post("/admin/sales/sales-invoice/change-status", {
                        invoice_id: invoice_id,
                        status: status,
                        _token: $('meta[name="csrf-token"]').attr('content')
                    }, function (res) {
                        document.location = "{{ route('admin.sales.sales-invoice.index') }}";
                    });
