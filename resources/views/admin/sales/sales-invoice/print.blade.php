...
<body onload="printAndClose()">
    ...
    <table class="details-table">
        <tr>
            <td width="33%">
                <strong>Doc. #:</strong> {{ $invoice->doc_no }}<br>
                <strong>Doc. Date:</strong> {{ $invoice->doc_date }}<br>
                <strong>Status:</strong> {{ ucfirst($invoice->status) }}
            </td>
            <td width="33%">
                <strong>Customer Name:</strong> {{ optional($invoice->customer)->customer_name }}<br>
                <strong>Contact No.:</strong> {{ $invoice->invoice?->phone_number }}<br>
                <strong>Address:</strong> {{ $invoice->invoice?->main_address }}
            </td>
            <td width="33%">
    ...
    <table class="summary-table">
        <tr>
            ...
            <td class="txt-right">{{ number_format($invoice->total_amount, 2) }}</td>
        </tr>
        <tr>
            <td><strong>CGST:</strong></td>
            <td class="txt-right">{{ number_format($invoice->cgst_amount, 2) }}</td>
        </tr>
        <tr>
            <td><strong>SGST:</strong></td>
            <td class="txt-right">{{ number_format($invoice->sgst_amount, 2) }}</td>
        </tr>
        <tr>
            <td><strong>IGST:</strong></td>
            <td class="txt-right">{{ number_format($invoice->igst_amount, 2) }}</td>
        </tr>
        <tr class="bold" style="border-top:1px solid #000;" >
            <td><strong>Grand Total:</strong></td>
            <td class="txt-right">{{ number_format($invoice->grand_amount, 2) }}</td>
        </tr>
    </table>
