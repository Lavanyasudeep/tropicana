...
<div class="page-sub-header">
    ...
    <div class="action-btns" >
        ...
        <a href="{{ route('admin.inventory.inward.create') }}" class="btn btn-create" title="create"><i class="fas fa-plus" ></i> Create</a>
    </div>
</div>

<!-- Advance Filter -->
<div class="page-advance-filter" id="inAdvFilterPanel" >
    <form id="inAdvFilterForm" >
        <div class="row">
            <div class="col-md-3" >
...
<div class="card page-list-panel" >
    <div class="card-body">
        <table id="inListTable" class="page-list-table">
            <thead>
                <tr>
                    <th>Doc. No.</th>
                    <th>Doc. Date</th>
...
$(document).ready(function() {
    ...
    $(document).on("click", '#hdrAdvFilterBtn', function(e) {
        e.preventDefault();
        if($('#inAdvFilterPanel').is(':visible'))
            $('#inAdvFilterPanel').hide();
        else
            $('#inAdvFilterPanel').show();
    });

    $(document).on("click", "#cancelFltrBtn", function () {
        const form = $('#inAdvFilterForm');
    ...
    $(document).on("click", "#closeFltrBtn", function () {
        ...
        document.getElementById('inAdvFilterPanel').style.display = 'none';
    });

    $(document).on("submit", '#inAdvFilterForm', function(e) {
        e.preventDefault();
        table.ajax.reload();
    });
    /***** End : Advance Filter *****/

    let table =  $('#inListTable').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: '{{ route("admin.inventory.inward.index") }}',
            data: function (d) {
        ...
        columns: [
            { data: 'doc_no', name: 'doc_no', width: '15%', orderable: true, searchable: true },
    ...
    $(document).on("submit", '#inListTable', function(e) {
        e.preventDefault();
        table.ajax.reload();
    });
    ...
    $(document).on('click', '.delete-btn', function(e) {
        e.preventDefault();
        let inwardId = $(this).data('id');

        Swal.fire({
            title: 'Are you sure?',
            text: "You want to cancel this Inward. This cannot be undone.",
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '/admin/inventory/inward/' + inwardId,
                    type: 'DELETE',
                    data: {
                        _token: '{{ csrf_token() }}'
                    },
                    success: function(response) {
                        table.ajax.reload();
                        Swal.fire('Cancelled!', 'Inward has been cancelled.', 'success');
                    },
                    error: function(xhr) {
