...
<div class="page-sub-header">
    ...
    <div class="action-btns" >
        ...
        <a href="{{ route('admin.inventory.gatepass.create') }}" class="btn btn-create" title="create" ><i class="fas fa-plus" ></i> Create</a>
    </div>
</div>
...
<div class="card page-list-panel" >
    <div class="card-body">
        <table id="gatePassListTable" class="page-list-table">
            <thead>
                <tr>
                    <th>Doc. No.</th>
                    <th>Doc. Date</th>
...
$(document).ready(function() {
    ...
    $(document).on("click", '#hdrAdvFilterBtn', function(e) {
        e.preventDefault();
        if($('#gatePassAdvFilterPanel').is(':visible'))
            $('#gatePassAdvFilterPanel').hide();
        else
            $('#gatePassAdvFilterPanel').show();
    });

    $(document).on("click", "#cancelFltrBtn", function () {
        const form = $('#gatePassAdvFilterForm');
    ...
    $(document).on("click", "#closeFltrBtn", function () {
        ...
        document.getElementById('gatePassAdvFilterPanel').style.display = 'none';
    });

    $(document).on("submit", '#gatePassAdvFilterForm', function(e) {
        e.preventDefault();
        table.ajax.reload();
    });
    /***** End : Advance Filter *****/

    let table =  $('#gatePassListTable').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: '{{ route("admin.inventory.gatepass.index") }}',
            data: function (d) {
    ...
    $(document).on("submit", '#gatePassListTable', function(e) {
        e.preventDefault();
        table.ajax.reload();
    });
